/* libbash.c -- teknopauls hack of shell.c to expose bash as a library. */
...
void libbash_set_variable(const char* name, char* value) {
	...
	strcpy(copy, value);
	bind_variable(name, copy, 0);
}

void libbash_get_variable(const char* name, char* value) {
	SHELL_VAR * bash_value = find_variable(name);
	if ( bash_value != (SHELL_VAR *)0 ) {
		strcpy(value, bash_value->value);
	}
}

// peak at a varaible value is not copied and should not be freed
char* libbash_peak_variable(const char* name) {
	SHELL_VAR * bash_value = find_variable(name);
	if ( bash_value != (SHELL_VAR *)0 ) {
		return bash_value->value;
	}
	return NULL;
}
...
/**
 * Returns a malloced linked list of visible shell variables from bash.
 * The list returned is filtered to exclude anything except
 * variables that start with an uppper case ASCII letter.
 * since these seem to be Bash internal stuff.
 *
 * Caller should free() this object using libbash_free_vars()
 */
struct bash_var * libbash_get_all_variables() {
	struct bash_var *bvar = NULL;
	struct bash_var *start = NULL;
	
	SHELL_VAR ** all_vars = all_visible_variables();
	int i = 0;
	for ( ; all_vars[i]; i++) {
		SHELL_VAR * var = all_vars[i];
		
		char first =  *var->name ;
		if ( first < 'A' || first > 'Z' ) continue;
		
		if (bvar == NULL) {
			bvar = malloc(sizeof(struct bash_var));
			start = bvar;
		}
		else {
			bvar->next = malloc(sizeof(struct bash_var));
			bvar = bvar->next;
		}
		
		//bvar->name = malloc(sizeof(char) * strlen( all_vars[i]->name + 1));
		//strcpy(bvar->name, all_vars[i]->name);
		bvar->name = all_vars[i]->name;
		if (all_vars[i]->value) {
			//bvar->value = malloc(sizeof(char) * strlen( all_vars[i]->value + 1));
			//strcpy(bvar->value, all_vars[i]->value);
			bvar->value = all_vars[i]->value;
		}
		else {
			bvar->value = NULL;
		}
...
struct bash_job* libbash_get_jobs() {
	struct bash_job *b_job = NULL;
	struct bash_job *start = NULL;
	
	int  j = 0;
	while (1) {
		// TODO surely we need some kinda mutex on this object
		JOB *job = jobs[j++];
		if (job) {
			if (b_job == NULL) {
				b_job = malloc(sizeof(struct bash_job));
				start = b_job;
			}
			else {
				b_job->next = malloc(sizeof(struct bash_job));
				b_job = b_job->next;
			}
			//char *command = malloc(sizeof(char) * strlen(job->pipe->command) );
			//strcpy(command, job->pipe->command);
			b_job->command = job->pipe->command;
			b_job->pid = job->pipe->pid;
			b_job->running = job->pipe->running;
			b_job->state = job->state;
			b_job->next = NULL;
		}
		else {
			//printf("here %d\n", j);
...
/**
 * This was bash's main() it is not changed much.
 * Tthe readloop is commented at the end of this method, 
 * so it returns without waiting for input
 */
...
/*  dont exec a command from the command line
...
/* dont open the script
  // Get possible input filename and set up default_buffered_input or default_input as appropriate.
  if (arg_index != argc && read_from_stdin == 0)
    ...
    // In this mode, bash is reading a script from stdin, which is a pipe or redirected file.
#if defined (BUFFERED_INPUT)
...
*/
  ...
  /* Bind remaining args to $1 ... $n */ // TODO off by one here?
     ...
     int extra;
{
  ...
  if (extra)
    fprintf (fp, _("GNU bash, version %s\n"), shell_version_string () );
